...
/* NOTES: Each constant corresponds to different values inside of the control
    ...
    kT -- relates the thrust generated by the motors to the square of the motor
    speed. Its units are [N/(speed^2)], where the speed units are to be defined.
    Some examples could be RPM, Servo input, Hz, rad, etc.

    kI -- describes the inertia of the quad to its rotation. In theory, the value
    is (3m + M)^-1, where m is the mass of the motor and M the mass of the frame
    arm. Its units are [kg/m^2], although it is relating the torque to the
    angular acceleration in this direction.

    kY -- describes how yaw is affected by the difference in motor speeds. This
    is theorietically possible to define rigidly, but it is best left as a
    tunable parameter to be adjusted according to sensitivity in this area.

    Nmax -- the maximum output of the CTM, as defined when all control variables
    are positive and add together. It will be used later.
...
void controlTransfer(unsigned int scaledInRoll, unsigned int scaledInPitch, unsigned int scaledInThrottle, unsigned int scaledInYaw) {
  ...
  if(U<=TCut){
       ...
       N1 = ((kT*U)/4)+((kI*R)/2)+((kY*Y)/4);
       N2 = ((kT*U)/4) + ((kT*P)/2) - ((Y * kY)/4);
       N3 = ((kT*U)/4)-((kI*R)/2)+((kY*Y)/4);
       N4 = ((kT*U)/4) - ((kT*P)/2) - ((Y * kY)/4);
  }
